...
def _find_best_checkpoint(run_dir: Path) -> Path:
    candidates = sorted(
        run_dir.rglob("best.ckpt"), key=lambda path: path.stat().st_mtime
    )
    if not candidates:
...
def _load_binary_predictions(path: str | Path) -> pd.DataFrame:
    ...
    if not required.issubset(data.files):
        raise ValueError(
            f"Prediction artifact {path} is missing {sorted(required - set(data.files))}."
        )
    ...
    if "labels" in data.files:
        ...
        if "binary_mask" in data.files:
            ...
            subject_ids, probabilities, labels = (
                subject_ids[mask],
                probabilities[mask],
                labels[mask],
            )
    ...
    for cohort_fine, cohort_cfg in sweep["cohorts"].items():
        ...
        for outcome in selected_outcomes:
            ...
            eligibility = eligibility_file_path(
                data_dir, cohort_fine, outcome, outcome_cfg
            )
            ...
            contexts.append(
                {
                    "cohort_fine": cohort_fine,
                    "cohort_grouped": labels["cohort_grouped"].dropna().iloc[0]
                    if not labels.empty
                    else cohort_cfg.get("clinical_group"),
            ...
            if dry_run and len(contexts) >= int(
                config.get("dry_run", {}).get("max_tasks", 2)
            ):
                return contexts
    return contexts
    ...
    for context in contexts:
        ...
        decision = classify_task_eligibility(
            ...
            max_test_indeterminate_fraction=float(
                rules["max_test_indeterminate_fraction"]
            ),
        )
        rows.append(
            {
...
def _sampled_outcome(
    ...
    selected = manifest[
        (manifest["sample_size"] == sample_size) & (manifest["seed"] == seed)
    ]
...
def _run_neural_cell(
    ...
    checkpoint = _format(
        model_cfg["encoder_ckpt"],
        cohort=context["cohort_fine"],
        clinical_group=context["clinical_group"],
        outcome=context["outcome"],
        seed=seed,
    )
    ...
    finetune = build_finetune_cmd(
        ...
        outcome_name=context["outcome"],
        outcome_path=str(outcome_path),
        output_dir=cell,
        base_config=model_cfg.get("base_config", "opera/configs/finetune.yaml"),
        ...
        competing_outcome_path=context["competing_path"],
        eligibility_path=context["eligibility_path"],
        registry_start_date=context["registry_start_date"],
        extra_overrides=[f"seed={seed}"],
    )
    if not execute:
...
def _run_tabular_cell(
    ...
    train_cmd = build_tabular_fraction_cmd(
        ...
        output_dir=cell / "train",
        cohort=context["cohort_fine"],
        outcome_name=context["outcome"],
        seed=seed,
        models=model_cfg.get("model", "xgboost"),
        tune=False,
        n_hours_start_include=context["outcome_cfg"].get("n_hours_start_include", 1),
        n_hours_end_include=context["outcome_cfg"].get("n_hours_end_include"),
        eligibility_path=context["eligibility_path"],
    ...
    eval_cmd = build_prediction_evaluate_cmd(
        predictions_path=str(candidates[0]),
        cohort=context["cohort_fine"],
        outcome_name=context["outcome"],
        outcome_path=str(outcome_path),
        model_family=model_name,
...
def _metadata_for_sample(
    ...
    return {
        ...
        "horizon_hours": context["outcome_cfg"].get("n_hours_end_include"),
        "sample_size": size,
        "total_target_labels_available": int(eligibility_row["n_train_determinate"]),
        "target_training_patients": int(train["subject_id"].nunique()),
        "target_tuning_patients": int(tuning["subject_id"].nunique()),
        "training_positives": int((train["label"] == 1).sum()),
        "training_negatives": int((train["label"] == 0).sum()),
        "tuning_positives": int((tuning["label"] == 1).sum()),
        "tuning_negatives": int((tuning["label"] == 0).sum()),
        "test_positives": int(eligibility_row["n_test_positive"]),
        "test_negatives": int(eligibility_row["n_test_negative"]),
        "test_indeterminate": int(eligibility_row["n_test_indeterminate"]),
        "seed": seed,
        "train_patient_hash": train["patient_id_hash"].iloc[0]
        if not train.empty
        else None,
        "tuning_patient_hash": tuning["patient_id_hash"].iloc[0]
        if not tuning.empty
        else None,
    }
    ...
    *,
    ...
    for context in contexts:
        eligibility_row = eligibility[
            (eligibility["cohort_fine"] == context["cohort_fine"])
            & (eligibility["outcome"] == context["outcome"])
        ].iloc[0]
        if not bool(eligibility_row["synthetic_eligible"]):
            continue
        n_available = int(eligibility_row["n_train_determinate"])
        sizes = resolve_sample_sizes(
            n_available,
            config["sampling"]["absolute_sizes"],
            config["sampling"].get("percentage_sizes", []),
        )
        if dry_run:
        ...
        for seed in seeds:
            manifest, skipped = build_nested_sample_manifest(
                ...
                downsample_tuning=bool(
                    config["sampling"].get("downsample_tuning", False)
                ),
            )
            ...
            if not skipped.empty:
                skipped_rows.append(
                    skipped.assign(
                        cohort_fine=context["cohort_fine"], outcome=context["outcome"]
                    )
                )
            for size in (
                sorted(manifest["sample_size"].unique()) if not manifest.empty else []
            ):
                sample_root = (
                    output
                    / context["cohort_fine"]
                ...
                sampled_outcome = _sampled_outcome(
                    context,
                    manifest,
                    int(size),
                    seed,
                    sample_root / "sampled_outcome.parquet",
                )
                metadata = _metadata_for_sample(
                    context, manifest, int(size), seed, eligibility_row
                )
                ...
                for model_name, model_cfg in models.items():
                    ...
                    table = bootstrap_metric_table(
                        prediction, n_bootstrap=n_bootstrap, seed=seed
                    )
                ...
                if predictions_by_model:
                    current = (
                        next(iter(predictions_by_model.values()))[
                            ["subject_id", "label"]
                        ]
                        .drop_duplicates()
                ...
                for model_name, prediction in predictions_by_model.items():
                    for comparator in comparators:
                        if (
                            ...
                            or comparator not in predictions_by_model
                        ):
                            continue
    ...
    manifests = (
        pd.concat(manifest_rows, ignore_index=True) if manifest_rows else pd.DataFrame()
    )
    skipped = (
        pd.concat(skipped_rows, ignore_index=True) if skipped_rows else pd.DataFrame()
    )
    metrics = (
        pd.concat(metric_rows, ignore_index=True) if metric_rows else pd.DataFrame()
    )
    differences = (
        pd.concat(difference_rows, ignore_index=True)
        if difference_rows
        else pd.DataFrame()
    )
    ...
    summary = (
        macro_synthetic_summary(metrics, n_bootstrap=n_bootstrap)
        if not metrics.empty
        else pd.DataFrame()
    )
    difference_summary = (
        macro_synthetic_summary(
            differences.rename(columns={"difference": "estimate"}),
            n_bootstrap=n_bootstrap,
        )
    ...
    savings = (
        estimate_label_savings(
            ...
            target=config.get("label_saving", {}).get("target_auroc"),
        )
...
def run_natural(
    config: dict, contexts: list[dict], eligibility: pd.DataFrame, *, dry_run: bool
) -> None:
    ...
    if dry_run:
        seeds = seeds[: int(config.get("dry_run", {}).get("max_seeds", 1))]
    n_bootstrap = int(
        config.get("dry_run", {}).get("bootstrap_replicates", 20)
        if dry_run
        else config["bootstrap_replicates"]
    )
    for context in contexts:
        eligibility_row = eligibility[
            (eligibility["cohort_fine"] == context["cohort_fine"])
        ...
        for seed in seeds:
            ...
            for model_name, model_cfg in config["models"].items():
                ...
                metric_rows.append(
                    table.assign(
                        ...
                        outcome=context["outcome"],
                        seed=seed,
                        natural_viability_tier=eligibility_row[
                            "natural_viability_tier"
                        ],
                        n_train_determinate=eligibility_row["n_train_determinate"],
                    )
                )
            ...
            if predictions:
                current = (
                    next(iter(predictions.values()))[["subject_id", "label"]]
                    .drop_duplicates()
                    .sort_values("subject_id")
                    .reset_index(drop=True)
                )
...
def regenerate_plots(config: dict, eligibility: pd.DataFrame) -> None:
    ...
    write_rarity_experiment_plots(
        ...
        synthetic_difference_summary=load(
            root / "synthetic" / "aggregate_relative_benefit.csv"
        ),
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--mode", choices=["synthetic", "natural", "plots", "all"], default="all"
    )
    parser.add_argument(
        "--execute",
        action="store_true",
        help="Run training/evaluation; otherwise only plan cells.",
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Limit tasks, seeds, sizes, and bootstrap replicates.",
    )
